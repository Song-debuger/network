...
public class Client {
    ...
    public static int handle(String line) throws RemoteException,
        ...
        if (line.equals("help")) {// 输入为help时打印菜单
            ...
            System.err.println("命令不存在");// 对于不存在的命令给出错误提示
            return 0;
        }
    }
        ...
        if (arg.length != 5) {// 参数个数不为5时，给出错误提示
            ...
            if (user2 == null) {// 如果未找到该用户，给出错误提示
                ...
                try {
                    ...
                    users.add(user);
                    users.add(user2);
                    meeting = new Meeting(meetingID, start, end, title, users);// 实例化会议
                    if (!meetingInterface.addMeeting(meeting)) {// 调用远程方法
                        System.err.println("会议时间重叠");// 远程方法会进行时间重叠检测
                    } else {
